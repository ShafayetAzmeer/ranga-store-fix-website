...
const showProducts = (products) => {
  const allProducts = products.map((item) => item);
    for (const product of allProducts) {
      ...
      div.innerHTML = `
        <div class="card col-11 single-product">
          <div class="mb-3"><img class="product-image img-fluid" src=${image}></img></div>
          <h3>${product.title}</h3>
          <p>Category: ${product.category}</p>
          <div class="d-flex justify-content-evenly fs-6 fw-bold mb-3">
            <span>Rating: ${product.rating.rate}</span>
            <span><i class="fas fa-user text-primary"></i> ${product.rating.count} </span>
          </div>
